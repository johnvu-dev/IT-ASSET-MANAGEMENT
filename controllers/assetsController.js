...
apiv1.get('/', async function(req, res){
    try {
        if (req.session.userId) {
            ...
            const Accessory = await Assets.findAll();
            //console.log(task);
            res.render('Assets_index', { Accessory });
          } else {
...
apiv1.get('/add', async (req, res) => {
    try {
        if (req.session.userId) {
            ...
            //console.log(task);
            res.render('Assets_add', { Category });
            } else {
            res.redirect('/auth/login');
            }
...
apiv1.post('/add', async (req, res) => {
    ...
    try {
        await Assets.create({ name, category_id });
        res.redirect('/assets');
    } catch (error) {
        console.error(error);
...
apiv1.post('/edit/:id', async (req, res) => {
        ...
        try {
            ...
            if (Asset) {
                await Asset.update({ name, category_id }); //err at 20230922
